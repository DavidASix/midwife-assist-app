...
class Calculator extends Component {
  ...
  onPressFrom(from) {
    ...
    LayoutAnimation.configureNext({ duration: 50, delete: { type: 'easeIn', springDamping: 0.4, property: 'scaleY' } });
    this.setState({ from });
  }
  ...
  renderInputRow() {
    ...
    switch(from) {
      case 'GA': {
        ...
        return (
          <>
            <View style={[styles.row, { borderWidth: 1, borderColor: 'transparent' }]}>
              <TextInput
                style={[styles.textInput, { borderColor: c.themes[theme].border, color: c.themes[theme].text }]}
 ...
 calc() {
   ...
   switch(from) {
     case 'GA': {
       ...
       gams = (((parseInt(weeks || 0) * 7) + parseInt(days || 0)) * c.t.day) + (Date.now() - gaRecDate);
       res = {
         ga: `${Math.floor(gams / c.t.week) || 0} weeks, ${Math.floor((gams % c.t.week) / c.t.day) || 0} days`,
     ...
     case 'LMP': {
       ...
       res = {
         ga: `${Math.floor(gams / (c.t.day * 7))} weeks, ${Math.round((gams % (c.t.day * 7)) / c.t.day)} days`,
    ...
    return (
      <View
        ...
        <View style={[styles.row]}>
          <Text style={[{ marginHorizontal: 10, color: c.themes[theme].text }, c.titleFont]}>
            GA:
          </Text>
          <Text style={[styles.textInput, { borderColor: c.themes[theme].border, maxWidth: 200, marginHorizontal: 10, color: c.themes[theme].text }]}>
            {res.ga}
          </Text>
        </View>

        {from !== 'LMP' && (<View style={[styles.row]}>
          <Text style={[{ marginHorizontal: 10, color: c.themes[theme].text }, c.titleFont]}>
            LMP:
          </Text>
          <Text style={[styles.textInput, { borderColor: c.themes[theme].border, maxWidth: 200, marginHorizontal: 10, color: c.themes[theme].text }]}>
        ...
        {from !== 'EDD' && (<View style={[styles.row]}>
          <Text style={[{ marginHorizontal: 10, color: c.themes[theme].text }, c.titleFont]}>
            EDD:
          </Text>
          <Text style={[styles.textInput, { borderColor: c.themes[theme].border, maxWidth: 200, marginHorizontal: 10, color: c.themes[theme].text }]}>
  ...
  renderSlidingPanel() {
    ...
    return (
      <SlideUpModal
        ...
        <View style={{ flex: 1, minHeight: 20, width: '80%', justifyContent: 'center', alignItems: 'center' }}>
  ...
  render() {
    ...
    return (
      ...
      <View style={[styles.container, { backgroundColor: c.themes[theme].background }]}>
        ...
        <View style={[styles.body, { backgroundColor: c.themes[theme].modal, borderColor: c.themes[theme].border }]}>

          <View style={{ flex: 3, width: '100%', justifyContent: 'space-around', alignItems: 'center' }}>
            <View style={[styles.row]} style={{ padding: 5, width: '100%' }}>
              <Text style={[{ color: c.themes[theme].text, fontSize: 24, marginLeft: 20 }, c.titleFont]}>
                Enter
              </Text>
            </View>
            <View style={[styles.row]}>
              <TouchableOpacity
                ...
                style={[styles.button, { borderColor: c.themes[theme].border, backgroundColor: c.themes[theme].background }, from === 'EDD' && { elevation: 5 }]}>
                <Text style={[styles.buttonText, { color: c.themes[theme].text }, from === 'EDD' && { color: c.themes[theme].accent }]}>
                  EDD
                </Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                style={[styles.button, { borderColor: c.themes[theme].border, backgroundColor: c.themes[theme].background }, from === 'GA' && { elevation: 5 }]}>
                <Text style={[styles.buttonText, { color: c.themes[theme].text }, from === 'GA' && { color: c.themes[theme].accent }]}>
                  GA
                </Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                style={[styles.button, { borderColor: c.themes[theme].border, backgroundColor: c.themes[theme].background }, from === 'LMP' && { elevation: 5 }]}>
                <Text style={[styles.buttonText, { color: c.themes[theme].text }, from === 'LMP' && { color: c.themes[theme].accent }]}>
                  LMP
                </Text>
              </TouchableOpacity>
