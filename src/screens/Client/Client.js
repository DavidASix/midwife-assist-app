...
class Client extends Component {
  ...
  renderClient(i, client) {
    ...
    const row2 = client.address.street ? { label: 'Street Address', data: client.address.street } :
      client.phones.length ? { label: 'Phone Number', data: client.phones[0] } : { label: '', data: '' };
    return (
      <TouchableOpacity
        onPress={() => this.props.navigation.navigate('viewClient', { client })}
        style={[styles.card, { backgroundColor: c.themes[theme].modal, borderColor: c.themes[theme].border, zIndex: 30 }]}>
        <View style={[styles.titleRow, { borderColor: c.themes[theme].border }]}>
          <View style={{
            ...c.center, height: 35, width: 35,
            borderRadius: 25, borderWidth: 1, borderColor: c.themes[theme].border,
            backgroundColor: c.themes[theme].background }}>
            <Text style={[{ fontSize: 16, color: c.themes[theme].text }, c.titleFont]}>
              {client.name.first.charAt(0).toUpperCase()}{client.name.last.charAt(0).toUpperCase() || ''}
            </Text>
          </View>
          <Text style={[{ fontSize: 20, color: c.themes[theme].text, marginHorizontal: 10 }, c.titleFont]}>
        ...
        <View style={styles.contentRow}>
          <View style={{ flex: 3, height: '100%', justifyContent: 'center' }}>
            <View style={{ marginVertical: 5 }}>
              ...
              <Text style={{ marginLeft: 5, fontSize: 16, color: c.themes[theme].text, textWrap: 'none' }}>
                {row1.data}
              </Text>
            </View>
            <View style={{ marginVertical: 5 }}>
          ...
          <View style={{ flex: 1, ...c.center }}>
            <View style={{
              ...c.center,
              height: 60, width: 60,
              borderRadius: 30, borderWidth: 1, borderColor: c.themes[theme].border,
              backgroundColor: c.themes[theme].background }}>
              <Image
                source={icon}
  ...
  renderSectionHeader(section) {
    ...
    return (
      <View style={{
        ...
        marginVertical: 2.5,
        width: '95%', }}>
        <View style={{ ...c.center,  elevation: 3, borderRadius: 20, height: 40, paddingHorizontal: 15, minWidth: 40, flex: 0, backgroundColor: c.themes[theme].accent }}>
    ...
    switch (sortType) {
      ...
      case 'eddAsc': {
        ...
        clients.forEach((client, i) => {
          ...
          let month = new Date(client.edd).getMonth() < 10 ? `0${new Date(client.edd).getMonth()}` : new Date(client.edd).getMonth();
          ...
          sections[`${year}_${month}`] = sections[`${year}_${month}`] ? [...sections[`${year}_${month}`], client] : [client];
        });
        ...
        sections = Object.keys(sections).map((key, i) => (
          {
            title: `${key.split('_')[0]}, ${c.months[parseInt(key.split('_')[1])]}`,
      ...
      default: {
        ...
        clients.forEach((client, i) => {
          ...
          sections[client.name.last.charAt(0).toUpperCase()] = sec ? [...sec, client] : [client];
        });
        // Format sections for section list
        sections = Object.keys(sections).map((key, i) => ({ title: key, data: sections[key] }));
        // Sort sections alphabetically
        sections = sections.sort((a, b) => (a.title > b.title ? 1 : -1));
        // Sort data of each section by first name
        sections.forEach((section, i) => sections[i].data = section.data.sort((a, b) => (a.name.last.toLowerCase() > b.name.last.toLowerCase() ? 1 : -1)));
        return sections;
        break;
      }
    }
  ...
  renderComingSoonSwitch() {
    let { theme, clients } = this.props;/*
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <Text style={{ color: c.themes[theme].text, fontSize: 24 }}>
          Coming Soon
        </Text>
      </View>
    );*/
    ...
    return (
      <>
        <SectionList
          ...
          ListHeaderComponent={() => <View style={{ height: 2.5, width: '100%' }} />}
          ListFooterComponent={() => <View style={{ height: 2.5, width: '100%' }} />}
      ...
      <View style={[styles.container, { backgroundColor: c.themes[theme].background }]}>
        <View style={[styles.header, { backgroundColor: c.themes[theme].accent }]}>
          <View style={{ flex: 2, width: '100%', justifyContent: 'center' }}>
            <Text style={[c.titleFont, { color: c.themes[theme].lightText, fontSize: 36, marginLeft: 20 }]}>
              Clients
            </Text>
          </View>
          {/*<Dropdown
