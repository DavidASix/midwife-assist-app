...
export const storeNewClient = client => dispatch =>
  dispatch({type: types.STORE_CLIENT, payload: client});
...
export const updateClient = updatedClient => dispatch =>
  dispatch({type: types.UPDATE_CLIENT, payload: updatedClient});

// Provides baby object which contains the mothers ID
export const storeNewBaby = baby => dispatch =>
  dispatch({type: types.STORE_BABY, payload: baby});

// Changes the sort type of the clients on client page. This is done in Redux to allow for a persistent state on reload.
export const changeSortType = type => dispatch =>
  dispatch({type: types.CHANGE_SORT_TYPE, payload: type});
